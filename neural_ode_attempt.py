...
def main(
    ...
    if plot:
        ...
        plt.plot(ts, ys[1, 0, :], c="crimson", label = "Real y")
        plt.plot(ts, ys[2, 0, :], c="yellow", label = "Real z")
        model_y = model(ts, ys[0, 0])
        plt.plot(ts, model_y[0, 0], c="magenta", label="Model x")
        plt.plot(ts, model_y[1, 0], c="black", label = "Model y")
        plt.plot(ts, model_y[2, 0], c="green", label = "Model y")

        plt.legend()
        plt.tight_layout()
        plt.savefig("attempt_ode.png")
        #plt.show()
